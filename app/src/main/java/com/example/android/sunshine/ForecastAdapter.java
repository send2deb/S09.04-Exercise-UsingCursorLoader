...
//        String weatherForThisDay = mWeatherData[position];
        ...
        String friendlyDate = SunshineDateUtils.getFriendlyDateString(mContext, date, true);
        ...
        double minTemp = mCursor.getDouble(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP));
        double maxTemp = mCursor.getDouble(mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP));

        String minAndMaxTemp = SunshineWeatherUtils.formatHighLows(mContext, minTemp, maxTemp);

        String weatherDescription = friendlyDate + " - " + weatherCondition + " - " + minAndMaxTemp;
//      COMPLETED (8) Display the summary that you created above
//        forecastAdapterViewHolder.weatherSummary.setText(weatherForThisDay);
        forecastAdapterViewHolder.weatherSummary.setText(weatherDescription);
    }
